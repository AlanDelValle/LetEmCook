...
@section('content')
    <div class="flex-col max-w-7xl mx-auto py-6">        
        <div class="flex items-center justify-center space-x-5 mt-20 mb-3">
            <form action="{{ route('tags.index') }}" method="GET" class="form relative">
        ...
        @if ($tags->isEmpty())
            <p class="text-center text-gray-400">Nenhum tema encontrado.</p>
        @else
            @include('partials.tag-grid', ['tags' => $tags])
        @endif        
    </div>    

<div class="mt-6 text-center">
    {{ $tags->links('layouts.pagination') }}
</div>
@endsection